...
no_img_pr ="""
give details in json format about the plant named above 
1. name of this plant scientific and common which ever i did not provide
...
st.subheader("Use Image or name of the plant to learn about")

use_img = st.toggle("I have the image of plant", value=False)
if (use_img and not(gen_exist("generate_iden"))):
   print("true after toggle -need upload")
   st.session_state["need_upload_iden"] = True
...
if generate:
  ...
  st.session_state.generate_iden = True

if gen_exist("generate_iden") and gak != "":
    # take model input
    if use_img:
        if len(uplo)==1:
    ...
    if gak =="":
        ...
        "temperature": 0.9,
        "top_p": 0.95,
        "top_k": 20, # the api change today 24 Nov,2024 onlu allows for between 1-41 tokens it was 64 before so gave error
    ...
    if st.button("Generate another"):
       ...
       del st.session_state["generate_iden"]
       del uplo
       del names
       st.rerun()
